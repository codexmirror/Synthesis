...
- `core/game` contains pure domain state and rules. It must not depend on React, browser APIs, the shell, or app UI. The React game context is only an adapter that exposes the canonical state to the client.
