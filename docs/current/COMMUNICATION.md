...
`203.0.113.42` — the Bookstore Gateway's own public entry address, never the
phone's private `10.42.0.x` position — is stored in the message she sent. It is
not a stored Device reference resolved to `target.ip` or `target.publicAddress`
at render time. If that address later changes, or the Device it fronts stops
being represented at all, the old message still says exactly what Myra said.
...
| — | — |
| clear interest (`yes`, `interested`, `send it`, `what do you have`, `let me see`, `tell me`, or an address request) | the first target address `203.0.113.42` and limited context |
