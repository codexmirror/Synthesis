...
The VEYRA phone's own consumer Wallet (`docs/current/VEYRA_OS.md`) presents the same optional statement context in its own consumer visual language: `description` as the row's lead, `purpose`/`location` beneath it, and the direction word plus counterparty reference on their own line. It is the Wallet that actually surfaces this most visibly, because the seeded Bookstore Branch's settlement Account is the phone's own Account (`dollar-account-veyra-phone-v0`) — so a runtime Bookstore sale shows there, not on the local NODE-OS Wallet, unless the settlement Account is reconfigured to the local Account.
