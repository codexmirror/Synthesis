...
A Financial Account has a stable internal ID, a distinct Provider-scoped account reference, and an integer `balanceCents`. Initial state contains exactly six ordinary Accounts: the player's `dollar-account-local-v0` / `CD-1042-7781` with 125000 cents ($1,250.00); the separately phone-accessible `dollar-account-veyra-phone-v0` / `CD-3318-2204` with 34250 cents ($342.50), which the represented VEYRA phone's Session authorizes; the neutral retail-clearing `dollar-account-retail-clearing-v0` / `CD-9000-2000` with 80000 cents ($800.00); the dedicated `dollar-account-bookstore-treasury-v0` / `CD-4827-6109`; Atlas Distribution's dedicated `dollar-account-atlas-distribution-treasury-v0` / `CD-5721-6408`; and Northline Book Supply's `dollar-account-northline-book-supply-treasury-v0` / `CD-6843-1906`. All three Treasury Accounts begin with 0 cents and have no Credential, Financial Session, saved sign-in, Device, or Player relationship. Business designates them by stable identity; Civic Dollar alone owns their Account, balance, and Transaction truth. All are ordinary Provider Accounts; none is owned by a Device, by NODE-OS, by VEYRA, by a Company, or by a branch. An Account ID is distinct from Player, Device, Credential, login, Session, Transaction, and account-reference identity.
...
## Company equipment payment

The authorized Bookstore Coffee Machine purchase uses the existing
`settleValidatedCompanyPurchase` -> `executeCivicDollarMovement` path for exact
Bookstore Company Treasury -> Atlas Distribution Treasury movement. It creates
no Financial Session, Credential or Account ownership. Operations stores only
the causing Transaction reference. Coffee basket revenue and personal gratuity
continue through their separate checkout movements; see
[`BRANCH_COMMERCE.md`](BRANCH_COMMERCE.md) for commercial terms and composition.
