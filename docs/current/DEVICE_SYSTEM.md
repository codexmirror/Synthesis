...
The current World contains `home-net` with node-01 and `srv-01`, plus the private foreign LocalNetwork `remote-segment-01` (`10.42.0.0/24`) with the personal phone `host-phone-001`, `srv-02` (`host-lan-002`), and `ops-01` (`host-lan-003`). Its own Router, `router-foreign-001`, sits at its internal LAN position `10.42.0.1` — the address its own members see as their default Gateway — and is separately, externally reconnaissable at the public edge `203.0.113.42` (`docs/current/NETWORK_ACCESS.md` "Gateway Reachability V1"), where its own `exposures` forward `srv-02`'s GateSSH and RackUpdate Services, the phone's GateSSH Service, and ops-01's GateSSH Service each out to their own distinct external port. Every other Service on this segment (`srv-02`'s own Bookstore Backend included) stays private and unreachable by its own address without first reaching a Device already on that LocalNetwork. Myra's authored first-target mail communicates the Gateway's own public entry address (`203.0.113.42`), never the phone's private address, as historical text and creates no Discovery.
...
`host-phone-001` is the one represented ordinary personal Device: display name `Petra’s Phone`, address `10.42.0.61`, `RUNNING`/`CONNECTED`, and a member of `remote-segment-01` with `host-lan-002`. It owns VEYRA OS 4.1 Firmware (`firmware-veyra-os-v4-1`), concrete Mobile CPU / 6 GB hardware and CPU/RAM runtime baselines, its own filesystem, and its own installed-software inventory. It carries no `role`, because it is not a server, and owns its own `NetworkTransferCapacity` (2 MiB/s upload, 4 MiB/s download) like the other concretely represented Devices, so an existing transfer involving it is decided on real grounds rather than for want of a represented capability.

Its filesystem is represented and empty, rather than filled with invented personal content, and it owns an empty Authentication History like the other resource-capable Devices. Its installed-software inventory holds exactly one concrete installation: the `Business 1.0` client (`business` / `business-1.0` / `build-business-1.0-v0`), owned by `docs/current/FILES_SOFTWARE.md` like every other represented installation, and seeded on this Device alone. It is ordinary Device-owned software, not a VEYRA Firmware feature, not a launcher flag, and not authority over anything: what the phone may actually manage with it is the separate Company Administration truth owned by `docs/current/BRANCH_COMMERCE.md`, and what VEYRA presents for it belongs to `docs/current/VEYRA_OS.md`.
