...
The current World contains `home-net` with node-01 and `srv-01`, plus the private foreign LocalNetwork `remote-segment-01` (`10.42.0.0/24`) with the personal phone `host-phone-001`, `srv-02` (`host-lan-002`), and `ops-01` (`host-lan-003`). Its own Router, `router-foreign-001`, sits at the public address `203.0.113.42` and owns the NAT-style `exposures` (`docs/current/NETWORK_ACCESS.md` "Gateway Reachability V1") that forward its own GateSSH and RackUpdate Services out to `srv-02`; the segment's members, `srv-02` included, are otherwise private and unreachable by their own address without first reaching a Device already on that LocalNetwork. Myra's authored first-target mail communicates the phone's own private address as historical text and creates no Discovery.
