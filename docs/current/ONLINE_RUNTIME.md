...
The server owns one idempotently started 250 ms clock, independent of connected-client count. Each step advances shared Bookstore/world cadence once through `advanceGameState`, then advances each real Player's Process and Device-owned runtime through the same domain advancement owner without repeating Bookstore cadence. Stopped time is not caught up.
