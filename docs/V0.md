...
A separate Shell-owned visual anchor makes those HOLD intervals preserve the
last accepted presentation in visual viewport space. It captures the rendered
Shell position before editable focus motion, applies only transient translation
compensation for external browser drift, and removes that compensation before a
newly accepted editing or normal presentation is painted. Scale changes suspend
translation anchoring rather than being treated as page motion. This controller
does not classify viewport sensors or derive editing geometry, and NODE
applications and RACK-OS do not own competing viewport compensation.
...
as distinct entries, alongside NODE/RACK layout rects and compact visual-anchor
state. The overlay is rendered outside the transformed Shell so it remains
usable while compensation is active. No diagnostic listeners or recording are
installed without the query flag.
