...
software-package files, and executable files. Each concrete copy has a stable
filesystem-local `id`; `path` is its current location rather than identity. A
filesystem-owned monotonic counter allocates deterministic IDs, so cross-Device
references require both Device ID and file ID. The local Device's initial
contents consist only of the text file `/home/user/welcome.txt`.
...
NodeScan 1.1 artifact on the Experimental channel. Both RACK-OS Files and Terminal observe those canonical
