...
for each client session. The current schema version is 25.
...
NODE-OS, NodeScan (`scan`, `inspect`, and `analyze`), and the Basic Credential Toolkit
(`attack`). Software provider groups and their represented versions derive from
the local Device's installations. NodeScan Scan, Inspect, and Analyze operations are
unavailable when NodeScan is absent; Credential Access remains unavailable when
the Basic Credential Toolkit is absent. Inspect is exposed directly as
`inspect <ipv4|network-name>` and from remembered Device and LocalNetwork pages
in NodeScan. Both interfaces use the same synchronous application operation.
...
produced/credited NODE, and a derived rate, deliberately without a
percentage progress bar; a Download shows its artifact,
...
execution for exactly one represented program, NODE Miner
(`node-miner`, release `node-miner-1.0`); `srv-01` owns its distributable
copy at `/opt/releases/node-miner-1.0.bin`, downloadable through the same
RACK-OS `DOWNLOAD` flow as any other artifact. Local Files exposes RUN only
for a currently present, locally downloaded executable copy whose
`programId` and `releaseId` identify the supported NODE Miner 1.0 release;
other executables (including future releases of NODE Miner) show no RUN
action. Upload, Uninstall, and a Software Manager remain unimplemented. See
"Program Execution and NODE Mining" below for the RUN, continuous runtime,
and NODE accounting this introduces.
...
mutation; V1 always admits onto the player's local Device.
...
Crossing a fixed V1 tuning threshold (100 compute-seconds per whole NODE)
converts that remainder into cumulative whole `producedNode` on the
Process. Credit is a separate, same-step concern: newly produced NODE
credits the represented local NODE Wallet's integer `balanceNode` (and the
Process's own `creditedNode`) only when the Process's configured
`payoutAddress` currently equals that Wallet's `address` by exact string
match (accepted address text is retained without normalization); a
non-matching Miner still runs and still produces NODE, credits nothing, and is never retried or retroactively credited if the Wallet
address later happens to match. STOP consumes zero simulation time,
...
address and cumulative produced/credited NODE, and a derived current NODE/s
rate. It deliberately carries no percentage completion bar, since indefinite
work has no completion threshold to show. Its card offers STOP, invoking the
same canonical `stopNodeMiner` operation described above.
...
attribute, not Wallet identity), and a canonical integer `balanceNode`.
