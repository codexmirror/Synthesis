...
`UPLOAD` for the directory currently being browsed. System derives the displayed Device,
...
size). Upload requires an explicit absolute remote destination path. Three player
surfaces reach it, all through the same shared GameContext action and canonical
FileTransfer runtime: the remote-first RACK-OS Files workflow, a selected local
Files artifact, and the RACK-OS `upload` command.
...
own; Processes / Activity Monitor remains the transfer-progress surface. Completion creates an
