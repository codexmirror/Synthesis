...
for each client session. The current schema version is 26.
...
operation shows CPU, RAM, configured payout address, cumulative
produced/credited atomic NODE units, and a derived units/s rate,
...
`/home/user/downloads/node-miner-1.0.pkg` — it is not yet installed, has no
executable, and is not running. Installing it through local Files
...
successful install records NODE Miner 1.0 as installed software on that
...
`producedNodeUnits` on the Process. Credit is a separate, same-step concern:
newly produced units credit the represented local NODE Wallet's integer
`balanceNodeUnits` (and the Process's own `creditedNodeUnits`) only when the
Process's configured `payoutAddress` currently equals that Wallet's
`address` by exact string match (accepted address text is retained without
normalization); a non-matching Miner still runs and still produces NODE,
credits nothing, and is never retried or retroactively credited if the
Wallet address later happens to match. STOP consumes zero simulation time,
...
address, cumulative produced/credited atomic NODE units, and a derived
current units/s rate. It deliberately carries no percentage completion bar,
...
ID, real CPU/RAM allocation, payout address, produced/credited atomic
units, and derived rate). Terminal owns no separate Miner or runtime state:
...
The Wallet application presents both canonical balances and the NODE
address, visually distinguished, and owns no production or payout logic of
its own.
...
- the Device-owned installed-software inventory, each entry showing its
  represented name, version, channel where present, and stable release ID
