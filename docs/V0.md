...
The player's local Device owns a canonical read-only filesystem. Its initial
contents consist only of `/home/user/welcome.txt`.

The Files application begins at `/home/user`, derives its directory listing
from that filesystem, and can display canonical text-file content. Terminal
provides local `ls` and `cat` commands over the same filesystem truth.

Filesystem mutation is not implemented. RACK-OS provides read-only access to
the first foreign server's separate canonical filesystem.
