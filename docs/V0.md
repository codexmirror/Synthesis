...
`programId` is supported; other executables (including any future
unsupported program) show no RUN action. Upload, Uninstall, and a Software
Manager remain unimplemented. See "Program Execution and NODE Mining" below
for the RUN, continuous runtime, and NODE accounting this introduces.
...
the same canonical filesystem Files and Terminal already share, requires
its `programId` to be `node-miner`, and requires an explicit non-empty
NODE payout address (Files prefills the represented local NODE Wallet
...
Process exists it retains its own stable `programId`/`releaseId`
provenance and configured `payoutAddress`, and no longer depends on that
source file, so moving or deleting it afterward never affects the running
...
under the same executor-owned CPU/RAM model as finite Processes: `startProcess`'s
RAM admission check reserves its RAM, and shared executor advancement
allocates it the same equal split of CPU headroom as any other running
...
match; a non-matching Miner still runs and still produces NODE, credits
nothing, and is never retried or retroactively credited if the Wallet
