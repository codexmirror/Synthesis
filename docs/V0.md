...
for each client session. The current schema version is 27.
...
while completed Process cards remain visible in the matching filtered
history. The running Download card offers a compact CANCEL control that
...
completed Process cards offer REMOVE instead; these controls are never
offered on another card type.
...
produced and configured-payout atomic NODE units, and a derived units/s rate,
...
and current effective rate, and never claims Process CPU or RAM. Completed
Process history remains available, is presented more quietly than running
work, and stays clearable either individually or all at once. FileTransfer
has no completion history, and none is presented.
...
NODE-OS REMOVE and CLEAR controls affect only completed local-device Process
history and cannot remove retained history owned by another executor Device.
...
the REMOVE control offered only on completed Process cards; FileTransfer
still has no completion history, so a cancelled or completed transfer simply
stops appearing.
...
Device and creates exactly one concrete NODE Miner `ExecutableFile` at the
deterministic path `/usr/local/bin/node-miner`, leaving the package artifact
in place and starting no Process. Reinstalling the same already-installed
...
behavior is that it routes 90% of its gross production to the configured
payout address and diverts 10% to one NODE address embedded in the build
...
`payoutNodeUnits`, and `developerFeeNodeUnits`, and the two allocations
always sum to the gross total. Both allocations are derived from the
cumulative gross total (`floor(gross * 10 / 100)` for the developer share)
rather than from each tick's newly produced units, so the same total gross
production always allocates identically however advancement was chunked:
`advance(1000)` and `4 x advance(250)` yield the same 900/100 split.
...
represented `NodeEconomy` account. There is no fallback recipient — a Miner
...
`/var/log/node-miner/payout.log`. It is created by the first actual payout
...
occupying that path is never overwritten. The artifact belongs to the
...
address, cumulative gross produced and configured-payout atomic NODE units,
and a derived current units/s rate. It presents no developer fee or
...
ID, real CPU/RAM allocation, payout address, gross produced and
configured-payout atomic units, and derived rate). Terminal owns no separate Miner or runtime state:
...
itself into, so its diverted 10% reaches real economic state rather than
