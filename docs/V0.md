...
for each client session. The current schema version is 19.
...
for display only and never represents a transfer as a `GameProcess`. Activity
is filtered as ALL, OPERATIONS (Service Analysis and Credential Access), and
TRANSFERS (the current active Download). No other activity type is
represented. Filter badges count running activity only, while completed
Process cards remain visible in the matching filtered history.
...
Session, DeviceAccess, and any active Session-bound FileTransfer remain intact.
...
Remote-to-local Download is the first filesystem mutation, and is now real
elapsed network-transfer runtime rather than an immediate copy. With a
current, resolvable Remote Session, `startRemoteFileDownload` admits and
starts at most one canonical `FileTransfer` (`GameState.fileTransfer`); a
second Download attempted while one is active fails deterministically with
`transfer_in_progress`. Starting a transfer records the exact foreign file's
stable `sourceDeviceId` + `sourceFileId`, the local Device's stable
`destinationDeviceId`, and the destination
...
The canonical `gameAdvancement` boundary independently advances this
FileTransfer runtime alongside Process runtime on every elapsed-time step, so
a transfer progresses even while no Process is running or changing. Elapsed
...
exactly once, at the moment `bytesTransferred` reaches `bytesTotal`; V1 has no
partial-file representation, so no destination artifact exists before that
point. The transfer remains bound to the RemoteSession that started it and to
live endpoint availability: if that Session stops being the current active
Session (including on disconnect), if the local Device leaves `ONLINE`, if the
remote source `NetworkHost` goes offline, or if the source artifact or
destination location can no longer be safely resolved, the active transfer
aborts without creating a destination artifact; established `DeviceAccess`
survives a disconnect-triggered abort, and reconnecting starts a new Session
rather than resuming the old transfer.
