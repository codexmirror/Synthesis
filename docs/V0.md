...
known, SCAN SELF remains available as a secondary footer control instead of a
section competing with the space it produced.
...
adapter derives Upload or Download direction from canonical endpoint identity
and resolves its artifact and current rate through the
same `accessId`-based FileTransfer authority that runtime advancement uses
rather than through any RemoteSession. A matching active Session may contribute
only its retained connected address as a route label; after disconnect that
route is omitted rather than recovered from mutable World identity. Activity is filtered as ALL, OPERATIONS (Service Analysis,
Credential Access, Software Installation, Software Removal, and NODE Miner), and TRANSFERS (the
current active Upload or Download). No other activity type is represented. Filter badges count running activity only,
while Recent Activity cards for ended Processes and FileTransfers remain visible in the matching filtered history. The running transfer card offers a compact CANCEL control that
...
download or upload usage is the active transfer's direction-aware derived effective rate; both are presented
...
deliberately without a percentage progress bar; a FileTransfer shows its direction, artifact,
...
`help`, `clear`, `ip`, `ls`, `cat`, `upload`, and `disconnect`; Terminal and Files read the
...
RACK-OS Terminal syntax is `download <remote-absolute-file-path>` or
`upload <local-absolute-file-path> <remote-absolute-file-path>`, and RACK-OS
...
Local Files presents a generic REMOTE TRANSFER section for any selected
concrete file while a Remote Session is active. It shows that Session's
retained connected address, provides a visible editable remote destination
suggestion, and calls the same canonical Upload operation only after the player
presses UPLOAD. Its in-progress state matches the selected source by local
Device ID plus stable file ID, never by path alone.
Download and Upload copy rather than move, never silently overwrite (re-checked at
...
size). Upload requires an explicit absolute remote destination path. RACK-OS
