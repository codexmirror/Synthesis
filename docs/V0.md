...
for each client session. The current schema version is 22.
...
presentation classification derived at observation time, not a stored or
canonical entity; raw compute capacity is never exposed to the player. Enhanced
...
downgrading to NodeScan 1.0 and re-inspecting overwrites remembered evidence
with the shallower result, since that reflects what the currently installed
release can legitimately observe. Terminal and the graphical NodeScan
