...
NODE-OS Terminal and RACK-OS Terminal share a presentation-only interaction
foundation while retaining separate commands, output semantics, prompts, and
per-instance history. Both preserve a live draft during history navigation,
guard asynchronous submission without disabling the input, respect text
composition, and follow new output only while the player remains near the end
of the terminal history. Their output regions are the canonical terminal
scroll owners; opening either Terminal does not automatically focus its input.
The shared frame supplies the shrinkable output/intrinsic-input layout, Shell
scroll-owner registration, output containment and touch policy, and common
command-input browser bindings. OS adapters supply their own accessible input
name, prompt, output renderer, commands, and visual identity.
