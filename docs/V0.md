...
Known Space presents SELF as the origin of the map, carrying the address and
the Scan action that starts a fresh session's discovery, followed by known
Networks. Each Network is the entry to the Devices observed through that
relationship; NodeScan does not duplicate them in a global Device index. A
Device page retains its Network navigation context and presents observed Device
