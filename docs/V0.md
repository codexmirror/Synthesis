...
for each client session. The current schema version is 21.
...
progress rail. Every application surface uses the same gutter and the same
uppercase technical labels. Each application still keeps whatever layout is
specific to it. This primitive set is NODE-OS Firmware presentation and is not
a universal Firmware framework: RACK-OS remains deliberately foreign and owns
its own palette and structure.

Opened NODE-OS applications state their current subject in a shared masthead
directly under the Shell application title: Files shows the current path,
System and Terminal show the local Device, and the Activity Monitor names
itself. Applications bound to the local Device say so, which is how the suite
distinguishes the local operating context from RACK-OS.
...
and Credential Access), and TRANSFERS (the current active Download). No other
activity type is represented. Filter badges count running activity only,
...
invokes the canonical `cancelFileTransfer` operation; completed Process cards
offer REMOVE instead, and the two controls are never offered on the other
card type.
...
runtime supports: an operation shows its historical target, progress, CPU
allocation, RAM requirement, and concrete completed result, while a Download
shows its artifact, source-to-destination relationship, transferred and total
bytes, progress, and current effective rate, and never claims Process CPU or
RAM. Completed Process history remains available, is presented more quietly
than running work, and stays clearable either individually or all at once.
FileTransfer has no completion history, and none is presented.
...
identity and artifact-size shape, represented endpoint network transfer
capacity described above, and the canonical `FileTransfer` runtime described
above make the schema version 19.
