...
Known Space presents SELF as the origin of a lightweight relationship tree,
carrying the address and the Scan action that starts a fresh session's
discovery, followed by connected known Network branches. Each Network is the
parent presentation for Devices observed through that relationship; NodeScan
does not duplicate them in a global Device index. A known non-SELF Device may
be expanded locally to browse only its remembered Service children without
performing an observation; one branch is expanded at a time. Each Device also
retains an explicit path to its full detail page, and Service children open
their existing detail pages. A Device page retains its Network navigation
