...
RACK-OS 1.0 Firmware identity, and a distinct canonical filesystem initially
containing only `/srv/readme.txt` with `Service workspace.`. The other training
hosts remain shallow network representations.
...
installation or a running Process. Its presence does not install or upgrade
NodeScan: downloading the experimental package leaves the local Device's
NodeScan at 1.0 Standard, leaves Inspect unavailable, and does not change Help,
Scan, Analyze, or installed software. Basic Credential Toolkit 1.0 also remains
installed. Installation and Upload operations are not implemented. These
behavioral filesystem mutations do not change the GameState shape, so the
schema remains version 15.
