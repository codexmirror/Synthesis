...
for each client session. The current schema version is 33.
...
NODE-OS, NodeScan (`scan` and `analyze`, plus `inspect` when the installed release supports it), the Basic Credential Toolkit
...
Scan and Analyze are unavailable when NodeScan is absent. Inspect is additionally
unavailable under NodeScan 1.0 Standard and is supplied by NodeScan 1.1
Experimental; Credential Access remains unavailable when the Basic Credential
...
Known Space presents known Networks as the top-level branches of a lightweight
relationship tree. Expanding a Network locally reveals its legitimately known
member Devices, including SELF, along with the Network's Scan action and, when
supported by the installed release, Inspect action. Network expansion is only
presentation state and performs no observation. A fresh session still presents
SCAN SELF as the action that discovers Network relationships. Each Network is
the parent presentation for Devices observed through that relationship;
NodeScan does not duplicate them in a global Device index. A known non-SELF
Device may be expanded locally to browse only its remembered Service children
without performing an observation; one branch is expanded at a time. Each
Device also retains an explicit path to its full detail page, and Service
children open their existing detail pages. A Device page retains its Network navigation
context and presents observed Device
facts, known Network relationships, known Services, Access, and Device actions. A Service row
...
Experimental override (see "Software Removal" below) — prevents later Inspect
without erasing previously remembered enhanced evidence. A
...
Firmware/compute evidence, survive failed Inspect attempts and loss of the current Inspect capability, and
...
Credential Access, Software Installation, Software Removal, and NODE Miner), and TRANSFERS (the
current active Download). No other activity type is represented. Filter badges count running activity only,
...
Software Installation, Software Removal) shows its historical target or package, percentage
progress, CPU allocation, RAM requirement, and concrete completed result; the
...
INSTALLED, INSTALLABLE, INSTALLING or UNSUPPORTED — from the same canonical
installed software, running local software-installation Process state, and
...
from installed software, only once installation completes, while Scan and
Analyze remain its only player-facing operations. Inspect remains
unavailable.
...
truthful failed result. Local Files exposes RUN only for a currently
...
package remains. Upload and a Software Manager remain unimplemented. See
"Program Execution and NODE Mining" below for the RUN, continuous runtime,
CLI, and NODE accounting this introduces, and "Software Removal" below for
the finite-Process REMOVE this V1 adds.
...
referenced under "Inspect" above. Changing installed
NodeScan capability never touches Discovery or Knowledge: previously stored
Enhanced Inspect snapshots remain exactly as observed, while a *new*
...
Local Files exposes REMOVE from the same package-artifact view Software
Installation exposes INSTALL from, transitioning a supported installed
package through INSTALLED → REMOVING → back to INSTALLABLE (NodeScan) or
absent from installed software (NODE Miner), and blocks duplicate admission
while REMOVING. The Activity Monitor observes Software Removal like any other
...
surface. Its navigable details combine the Device-owned current installation
