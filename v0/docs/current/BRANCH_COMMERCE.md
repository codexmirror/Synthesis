...
Catalog validity requires every Baseline Popularity to be a positive finite safe integer. Baseline Popularity remains each Book's stable, Book-specific neutral demand baseline; it is not overwritten by current market context.

`GameState.bookstoreMarket` (`src/core/game/bookstoreMarket.ts`) separately owns global hidden Bookstore Genre Market Pressure World Truth. It contains exactly one positive safe-integer relative Pressure index for every represented Book Genre: `SCIENCE_FICTION = 120`, `THRILLER = 100`, `MYSTERY = 90`, and `LITERARY_FICTION = 100`. A Pressure of 100 is neutral, not a percentage or cap. Missing, duplicate, dangling, zero, negative, fractional, non-finite, or unsafe Pressure truth fails closed with no neutral fallback.

For each Book, current Effective Demand is derived as `baselinePopularity × Pressure for Book.genre` and used directly as its relative weighted-selection input. Effective Demand is never stored. Both every derived weight and the current sellable-candidate aggregate must be positive safe integers before purchase randomness is consumed. All Books in one Genre receive the same Pressure, preserving their Baseline Popularity ratios rather than flattening them.

Pressure applies only after eligibility remains the exact intersection of valid Catalog identity, Branch assortment, and positive physical stock. It changes only relative Book composition inside an already legitimate opportunity: basket-size probabilities, one basket-size draw, one `bookstorePurchaseRandom` draw per selected unit, provisional depletion, aggregation, settlement, and CompletedSale semantics are unchanged. It changes no cadence, price, Supplier Offer, procurement term, Treasury, assortment, or stock directly. `bookstoreSalesCadence` and its `bookstoreDemandRandom` remain solely responsible for opportunity scheduling and never read Pressure.

V1 Genre Market Pressure is temporally inert hidden World Truth. No Trend lifecycle, automatic Pressure change, history, Market Intelligence/Research, observation, or Player Knowledge is implemented. VEYRA Product Detail continues to expose the Book's Baseline Popularity as `Popularity`; it exposes neither Pressure nor derived Effective Demand.

Genre and Baseline Popularity are global Book truth, while Genre Market Pressure is separate global Bookstore market truth and stock and procurement history remain Branch truth. VEYRA may show Genre and the stable baseline index as ordinary carried-product merchandising information. Supplier terms never alter Genre, price, Popularity, or Market Pressure. Trends, stored Effective Demand, popularity drift, market intelligence, and modifier systems remain unimplemented.
