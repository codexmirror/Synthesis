...
function renderTerminal(scanTarget: GameActions['scanTarget']) {
  ...
  const actions: GameActions = {
    deliverRecovery: vi.fn(), authenticateServiceKey: vi.fn(), runSentrySweep: vi.fn(), surveyAddress: vi.fn(), pingTarget: vi.fn(),
    scanTarget,
    findTargets: vi.fn(), refreshNetwork: vi.fn(),
    ...
    openMailThread: () => {}, sendMailReply: () => ({ status: 'thread_unavailable', state }), composeMail: () => ({ status: 'unknown_recipient', state }), deleteMailThreads: () => {},
...
describe('Terminal credential access', () => {
  it('resolves remembered identities and invokes the same application action used by Scan', async () => {
    ...
    vi.spyOn(GameContext, 'useGameActions').mockReturnValue({
      deliverRecovery: vi.fn(), authenticateServiceKey: vi.fn(), runSentrySweep: vi.fn(), surveyAddress: vi.fn(), pingTarget: vi.fn(), scanTarget: vi.fn(), findTargets: vi.fn(), refreshNetwork: vi.fn(), startServiceAnalysis: vi.fn(), startServiceAnalysisAtEndpoint: vi.fn(), startServiceAnalysisFromObservation: vi.fn(), startObservedServiceAnalyses: vi.fn(),
...
describe('Terminal live Process projection', () => {
  it('binds Analyze to canonical Process state, updates in place, and preserves input focus', async () => {
    ...
    render(<GameProvider initialState={{ ...initial, knowledge: { bookstoreMarket: { nextReportId: 1, reports: [] }, discoveredVulnerabilities: [] } }}><Terminal /><StateControls /></GameProvider>)
    const user = userEvent.setup({ advanceTimers: vi.advanceTimersByTime })
  ...
  it('keeps concurrent entries independently bound and reflects canonical CPU sharing', async () => {
    vi.useFakeTimers({ shouldAdvanceTime: true })
    const initial = { ...knownCredentialState(), knowledge: { bookstoreMarket: { nextReportId: 1, reports: [] }, discoveredVulnerabilities: [] } }
