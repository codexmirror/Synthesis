...
export const installCommand: TerminalCommand = {
  ...
  run(context, args) {
    ...
    switch (result.status) {
      case 'started': return { type: 'output', lines: ['INSTALLING', `${result.name} ${result.version} ${titleCase(result.channel)}`, `PROCESS ${result.processId}`] }
      case 'already_installed': return { type: 'output', lines: ['ALREADY INSTALLED'] }
      case 'already_installing': return { type: 'output', lines: ['ALREADY INSTALLING'] }
      ...
      case 'insufficient_memory': return { type: 'output', lines: [`INSUFFICIENT MEMORY · REQUIRES ${result.requiredMiB} MiB`] }
    }
  },
}
