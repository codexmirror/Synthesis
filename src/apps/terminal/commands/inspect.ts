...
export const inspectCommand: TerminalCommand = {
  ...
  run: ({ operations }, args) => {
    ...
    if (result.status === 'capability_unavailable') return { type: 'output', lines: ['INSPECT UNAVAILABLE', '', 'Installed NodeScan does not support Inspect.'] }
    ...
    if (result.scope !== 'self' && result.displayName) lines.splice(1, 0, `Name:    ${result.displayName}`)
