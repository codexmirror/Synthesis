...
function ProcessProjection({ process, gameState, cpu }: { process?: GameProcess; gameState: ReturnType<typeof useGameState>; cpu: number }) {
  ...
  return <section className="process-projection" aria-label={`${process.label} ${process.status}`}>
    ...
    {(process.kind === 'credential_access' || process.kind === 'rack_update_exploit') && process.status === 'running' && <div className="muted">{process.kind === 'credential_access' && process.toolId === 'credential-access-module' ? FLIPPER_MODULE_NAME[process.moduleId] : `Flipper · ${FLIPPER_MODULE_NAME[process.moduleId]}`}</div>}
