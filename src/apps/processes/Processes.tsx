...
export function Processes() {
  ...
  const endpointFor = (process: typeof state.process.processes[number]) => {
    ...
    return host && service ? `${host.ip}:${service.port}` : 'Unavailable service'
  }
  const cards = (processes: typeof state.process.processes) => processes.map((process) => <article className="process-card" key={process.id}>
    ...
    {process.kind === 'service_analysis' && <p><span>Target</span><br /><strong className="process-value">{endpointFor(process)}</strong></p>}
    ...
    {process.result?.status === 'weaknesses_detected' && <p><strong>WEAKNESS DETECTED</strong><br />{process.result.vulnerabilities.map(({ vulnerabilityId, observedLabel }) => <span className="process-value" key={vulnerabilityId}>{observedLabel}</span>)}</p>}
    {process.result?.status === 'no_weakness_detected' && <p><strong>NO WEAKNESS DETECTED</strong></p>}
    {process.result?.status === 'service_unavailable' && <p><strong>SERVICE UNAVAILABLE</strong></p>}
  </article>)
