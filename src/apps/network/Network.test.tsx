...
describe('Scan workspace', () => {
  ...
  it('executes graphical Scan through the application operation without reading World', async () => {
    ...
    vi.spyOn(GameContext, 'useGameActions').mockReturnValue({
      ...
      connectRemoteFromObservation: vi.fn(), disconnectRemoteSession: vi.fn(), downloadRemoteFile: vi.fn(), clearCompletedProcesses: vi.fn(),
    })
  ...
  it('ignores an older rescan result after a newer device observation resolves', async () => {
    ...
    vi.spyOn(GameContext, 'useGameActions').mockReturnValue({ scanTarget, startServiceAnalysis: vi.fn(), startServiceAnalysisAtEndpoint: vi.fn(), startServiceAnalysisFromObservation: vi.fn(), startCredentialAccessAttemptFromObservation: vi.fn(), connectRemoteFromObservation: vi.fn(), disconnectRemoteSession: vi.fn(), downloadRemoteFile: vi.fn(), clearCompletedProcesses: vi.fn() })
    const user = userEvent.setup()
    render(<Network />)
    await user.click(await screen.findByRole('button', { name: 'Open known area home-net' }))
    delayNetwork = true
  ...
  it('binds Analyze to the observed endpoint and never silently retargets a stale card', async () => {
    ...
    vi.spyOn(GameContext, 'useGameActions').mockReturnValue({
      scanTarget: async (input) => scanNetworkTarget({ localDevice: canonical.player.localDevice, network: canonical.world.network }, input), startServiceAnalysis: vi.fn(), startServiceAnalysisAtEndpoint: vi.fn(), startServiceAnalysisFromObservation: endpointAction, startCredentialAccessAttemptFromObservation: vi.fn(), connectRemoteFromObservation: vi.fn(), disconnectRemoteSession: vi.fn(), downloadRemoteFile: vi.fn(), clearCompletedProcesses: vi.fn(),
    })
