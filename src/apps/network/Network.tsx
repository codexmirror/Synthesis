...
import { findInstalledBasicCredentialToolkit, findInstalledNodeScan, nodeScanSupportsInspect } from '../../core/game/software'
...
export function Network() {
  ...
  const nodeScan = findInstalledNodeScan(gameState.player.localDevice)
  const canInspect = Boolean(nodeScan && nodeScanSupportsInspect(nodeScan))
...
function DeviceView({ canInspect, observation, network, networks, pending, processes, knowledge, access, activeAccessId, connectionFeedback, copyState, onCopy, onScan, onInspect, onConnect, onDisconnect, onService, onBack }: { canInspect: boolean; observation: DiscoveredDeviceSnapshot; network: DiscoveredNetworkSnapshot | null; networks: readonly DiscoveredNetworkSnapshot[]; pending: boolean; processes: readonly ServiceAnalysisProcess[]; knowledge: readonly { targetDeviceId: string; serviceId: string; observedLabel: string }[]; access: readonly { id: string; viaServiceId: string; privilege: 'USER' }[]; activeAccessId?: string; connectionFeedback: ConnectionFeedback; copyState: CopyState; onCopy(value: string): void; onScan(): void; onInspect(): void; onConnect(): void; onDisconnect(): void; onService(service: DiscoveredServiceSnapshot): void; onBack(): void }) {
  const activeAccess = access.find(({ id }) => id === activeAccessId)
  return <div><nav className="scan-crumbs" aria-label="Scan navigation"><button onClick={onBack}>← {network?.name ?? 'Devices'}</button><span>/</span><strong>{observation.address}</strong></nav><div className="scan-detail-title"><div><span className="scan-type">{observation.scope.toUpperCase()} DEVICE</span><CopyReference value={observation.address} copyState={copyState} onCopy={onCopy} /></div></div>{observation.inspect && <><div className="node-section"><span>FACTS</span></div><dl className="service-facts"><div><dt>TYPE</dt><dd>{observation.inspect.deviceKind.toUpperCase()}</dd></div><div><dt>STATUS</dt><dd>{observation.inspect.networkStatus}</dd></div>{observation.inspect.enhanced && <><div><dt>FIRMWARE</dt><dd>{observation.inspect.enhanced.firmware.name} {observation.inspect.enhanced.firmware.version}</dd></div><div><dt>COMPUTE</dt><dd>{observation.inspect.enhanced.computeClass}</dd></div></>}</dl></>}{access.length > 0 && <><div className="node-section"><span>STATE</span></div><section className={`device-state interaction${activeAccess ? ' interaction-active' : ''}`} aria-label={activeAccess ? 'Remote session active' : 'Device access available'}>{activeAccess ? <><div className="interaction-copy"><strong>REMOTE SESSION</strong><span>{activeAccess.privilege} · ACTIVE</span></div><button className="interaction-action" onClick={onDisconnect}><span className="interaction-copy"><strong>DISCONNECT</strong></span><span className="atlas-arrow" aria-hidden="true">→</span></button></> : <button className="interaction-action" onClick={onConnect}><span className="interaction-copy"><strong>{access[0].privilege} ACCESS</strong><span>CONNECT</span></span><span className="atlas-arrow" aria-hidden="true">→</span></button>}</section>{connectionFeedback && <p className="scan-feedback" role="status">{connectionFeedback}</p>}</>}<div className="node-section"><span>NETWORKS</span><span>{networks.length} known</span></div>{networks.map((knownNetwork) => <div className="relationship" key={knownNetwork.id}><span>NETWORK</span><strong>{knownNetwork.name}</strong></div>)}<div className="node-section"><span>SERVICES</span><span>{observation.servicesObserved ? `${observation.services.length} known services` : 'Services not observed yet'}</span></div><div className="service-list">{observation.services.map((service) => {
    ...
    return <button type="button" className="service-row" key={service.id} onClick={() => onService(service)} aria-label={`Open ${service.name} service`}><span className="service-row-main"><strong>{service.name}</strong><span>{service.port} / {service.protocol}</span><span className="node-chip">KNOWN</span></span>{service.inspect && <span className="service-row-fingerprint"><span>{service.inspect.implementation.name} {service.inspect.implementation.version}</span>{service.inspect.authentication && <span>Authentication: {service.inspect.authentication}</span>}</span>}<span className="service-row-secondary"><span>{summary}</span><span className="service-row-arrow" aria-hidden="true">→</span></span></button>
  })}</div><div className="node-section"><span>ACTIONS</span></div>{canInspect && <button className="scan-device-action" onClick={onInspect}><strong>{observation.inspect ? 'INSPECT AGAIN' : 'INSPECT DEVICE'}</strong><span className="atlas-arrow" aria-hidden="true">→</span></button>}<button className="scan-device-action" disabled={pending} onClick={onScan} aria-label={`Scan device ${observation.address}`}><strong>SCAN DEVICE</strong><span className="atlas-arrow" aria-hidden="true">→</span></button></div>
}
