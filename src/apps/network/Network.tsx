...
export function Network() {
  ...
  function connect(targetDeviceId: string, address: string) {
    actions.connectRemoteFromObservation({ targetDeviceId, address })
  }
  ...
  return <section className="app-content scan-app" aria-label="Scan workspace">
    {serviceObservation && deviceObservation ? <ServiceView observation={serviceObservation} deviceAddress={deviceObservation.address} processes={gameState.process.processes.filter((p): p is ServiceAnalysisProcess => p.kind === 'service_analysis')} credentialProcesses={gameState.process.processes.filter((p): p is CredentialAccessProcess => p.kind === 'credential_access')} knowledge={gameState.knowledge.discoveredVulnerabilities} ownsToolkit={gameState.player.localDevice.tools.some(({ id }) => id === BASIC_CREDENTIAL_TOOLKIT_ID)} access={gameState.deviceAccess.established.find((item) => item.sourceDeviceId === gameState.player.localDevice.id && item.targetDeviceId === serviceObservation.targetDeviceId && item.viaServiceId === serviceObservation.id)} copyState={copyState} feedback={feedback} onCopy={copy} onAnalyze={analyze} onAttempt={attempt} onBack={() => { setServiceObservation(null); setFeedback(null) }} />
      : deviceObservation ? <DeviceView observation={deviceObservation} network={networkObservation} networks={deviceNetworks} pending={pendingTarget === deviceObservation.address} processes={gameState.process.processes.filter((p): p is ServiceAnalysisProcess => p.kind === 'service_analysis')} knowledge={gameState.knowledge.discoveredVulnerabilities} access={gameState.deviceAccess.established.filter((item) => item.sourceDeviceId === gameState.player.localDevice.id && item.targetDeviceId === deviceObservation.id)} activeAccessId={gameState.remoteSession.active?.accessId} copyState={copyState} onCopy={copy} onScan={() => scanDevice(deviceObservation.address)} onConnect={() => connect(deviceObservation.id, deviceObservation.address)} onDisconnect={() => actions.disconnectRemoteSession()} onService={(service) => { setServiceObservation({ ...service, targetDeviceId: deviceObservation.id }); setFeedback(null) }} onBack={() => { invalidateRequests(); setDeviceId(null); setFeedback(null) }} />
...
function DeviceView({ observation, network, networks, pending, processes, knowledge, access, activeAccessId, copyState, onCopy, onScan, onConnect, onDisconnect, onService, onBack }: { observation: DiscoveredDeviceSnapshot; network: DiscoveredNetworkSnapshot | null; networks: readonly DiscoveredNetworkSnapshot[]; pending: boolean; processes: readonly ServiceAnalysisProcess[]; knowledge: readonly { targetDeviceId: string; serviceId: string; observedLabel: string }[]; access: readonly { id: string; viaServiceId: string; privilege: 'USER' }[]; activeAccessId?: string; copyState: CopyState; onCopy(value: string): void; onScan(): void; onConnect(): void; onDisconnect(): void; onService(service: DiscoveredServiceSnapshot): void; onBack(): void }) {
  const activeAccess = access.find(({ id }) => id === activeAccessId)
  return <div><nav className="scan-crumbs" aria-label="Scan navigation"><button onClick={onBack}>← {network?.name ?? 'Devices'}</button><span>/</span><strong>{observation.address}</strong></nav><div className="scan-detail-title"><div><span className="scan-type">{observation.scope.toUpperCase()} DEVICE</span><CopyReference value={observation.address} copyState={copyState} onCopy={onCopy} /></div><button className="scan-quiet" disabled={pending} onClick={onScan} aria-label={`Scan device ${observation.address}`}>⌁ <span>Scan Device</span></button></div>{access.length > 0 && <><div className="scan-section-heading"><span>{activeAccess ? 'REMOTE SESSION' : 'USER ACCESS'}</span></div><section className="interaction"><button className="interaction-action" onClick={activeAccess ? onDisconnect : onConnect}><span className="interaction-copy"><strong>{activeAccess ? 'ACTIVE' : 'CONNECT'}</strong><span>{access[0].privilege}</span></span><span className="atlas-arrow" aria-hidden="true">→</span></button></section></>}<div className="scan-section-heading"><span>NETWORKS</span><span>{networks.length} known</span></div>{networks.map((knownNetwork) => <div className="relationship" key={knownNetwork.id}><span>NETWORK</span><strong>{knownNetwork.name}</strong></div>)}<div className="scan-section-heading"><span>SERVICES</span><span>{observation.servicesObserved ? `${observation.services.length} known services` : 'Services not observed yet'}</span></div><div className="service-list">{observation.services.map((service) => {
