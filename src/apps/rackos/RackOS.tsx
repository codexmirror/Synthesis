...
function RemoteFiles({ filesystem }: { filesystem: ActiveRemoteTarget['target']['filesystem'] }) {
  ...
  if (selected) return <section className="rack-files"><p>FILES</p><code>{selected}</code><button onClick={() => setSelected(undefined)}>← {path}</button>{result?.status === 'ok' && (result.file.kind === 'text' ? <pre>{result.file.content}</pre> : <div><p>SOFTWARE PACKAGE</p><h2>{result.file.name}</h2><p>{result.file.version} {titleCase(result.file.channel)}</p><dl><dt>PACKAGE</dt><dd>{result.file.packageId}</dd><dt>PATH</dt><dd>{result.file.path}</dd></dl></div>)}</section>
