...
export function SystemBar({ remoteContext, onReturnRemote }: { remoteContext?: ActiveRemoteTarget; onReturnRemote?(): void }) {
  ...
  return <footer className={`system-bar${remoteContext ? ' system-bar--remote' : ''}`}><span>CPU <strong>{Math.round(usage.totalCpuLoad)}%</strong></span><span>RAM <strong>{Math.round(usage.totalRamUsage)}%</strong></span><span className="online">NET <strong>{runtime.networkStatus}</strong></span>{connectedAddress && <button type="button" className="remote-context" aria-label={`RETURN REMOTE · ${connectedAddress}`} onClick={onReturnRemote}><span className="remote-context__label">RETURN REMOTE</span><span className="remote-context__address">{connectedAddress}</span></button>}</footer>
}
