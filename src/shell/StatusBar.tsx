...
export function StatusBar() {
  ...
  return <header className="status-bar"><strong className="brand">{device.firmware.name} <span>/</span> {device.displayName}</strong><div className="top-stats"><span>{device.network.ip}</span><time>{time.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</time><span className="connection" aria-label={`Network ${device.runtime.networkStatus}`}><i />{device.runtime.networkStatus}</span></div></header>
}
