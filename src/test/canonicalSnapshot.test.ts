...
describe('withoutBookstoreCadenceTiming', () => {
  it('normalizes only remainingUntilOpportunityMs, preserving branchId, opportunityIntervalMs, and record count', () => {
  ...
  it('still distinguishes a real change in opportunityIntervalMs', () => {
    const base = createInitialGameState()
    const reconfigured: GameState = { ...base, bookstoreSalesCadence: { records: base.bookstoreSalesCadence.records.map((record) => ({ ...record, opportunityIntervalMs: 5_000 })) } }
