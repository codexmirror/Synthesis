...
describe('filesystem reads and identity', () => {
  it('derives only direct directory children from canonical paths', () => {
    ...
    expect(listDirectory(filesystem, '/home/user')).toEqual({ status: 'ok', entries: [{ name: 'downloads', type: 'directory' }, { name: 'modules', type: 'directory' }, { name: 'welcome.txt', type: 'file' }] })
  })
