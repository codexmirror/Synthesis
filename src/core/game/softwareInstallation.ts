...
import { RATTLER_EXECUTABLE_SIZE_BYTES, RATTLER_INSTALLED_EXECUTABLE_PATH, RATTLER_PRODUCT_ID } from './rattler'
...
function admitSoftwareInstallation(process: ProcessState, target: SoftwareInstallationTarget, packagePath: string): AdmitInstallationResult {
  ...
  const managedPath = packageFile.productId === NODE_MINER_PROGRAM_ID ? NODE_MINER_INSTALLED_EXECUTABLE_PATH : packageFile.productId === RATTLER_PRODUCT_ID ? RATTLER_INSTALLED_EXECUTABLE_PATH : undefined
  if (managedPath && checkDestinationPlacement(target.filesystem, managedPath) !== 'ok') {
...
function applyInstallationCompletion(device: InstallationOwnedState, process: SoftwareInstallationProcess): InstallationOwnedState & { readonly result: SoftwareInstallationResult } {
  ...
  const executablePath = process.productId === FLIPPER_PRODUCT_ID ? FLIPPER_INSTALLED_EXECUTABLE_PATH : process.productId === RATTLER_PRODUCT_ID ? RATTLER_INSTALLED_EXECUTABLE_PATH : NODE_MINER_INSTALLED_EXECUTABLE_PATH
  ...
  const executable: ExecutableFile = {
    ...
    sizeBytes: process.productId === FLIPPER_PRODUCT_ID ? FLIPPER_EXECUTABLE_SIZE_BYTES : process.productId === RATTLER_PRODUCT_ID ? RATTLER_EXECUTABLE_SIZE_BYTES : NODE_MINER_EXECUTABLE_SIZE_BYTES,
  }
  return {
