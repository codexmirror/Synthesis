...
describe('Service Analysis', () => {
  it('resolves endpoint syntax to stable process identity and rejects duplicates', () => {
    ...
    expect(first.state.process.processes[0]).toMatchObject({ kind: 'service_analysis', targetDeviceId: 'host-lan-001', serviceId: 'service-ssh-001', executorDeviceId: 'device-local-v0' })
  ...
  it('resolves exactly once, adds positive knowledge once, and allows re-analysis', () => {
    ...
    expect(twice.process.processes[0].result).toBe(process.result)
  })
  it('reads current vulnerability truth only at completion and retains historical knowledge', () => {
    ...
    const done = advanceGameState(changed, 20_000); expect(done.process.processes[0].result).toEqual({ status: 'no_weakness_detected' }); expect(done.knowledge.discoveredVulnerabilities).toEqual([])
  ...
  it('reports unavailable from current truth, while stable identity survives a port change', () => {
    ...
    expect(advanceGameState(closed, 20_000).process.processes[0].result).toEqual({ status: 'service_unavailable' })
    ...
    expect(advanceGameState(moved, 20_000).process.processes[0]).toMatchObject({ serviceId: 'service-ssh-001', result: { status: 'weaknesses_detected' } })
  })
  it('HTTP detects zero weaknesses without negative knowledge', () => {
    const done = advanceGameState(started('service-http-001'), 20_000); expect(done.process.processes[0].result).toEqual({ status: 'no_weakness_detected' }); expect(done.knowledge.discoveredVulnerabilities).toEqual([])
  })
})
