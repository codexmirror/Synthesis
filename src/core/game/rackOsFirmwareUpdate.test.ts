...
/**
 * This file exercises RACK-OS firmware update, an unrelated domain from
 * Bookstore Sales Cadence. Its own install/reboot cycles advance canonical
 * elapsed time well past 30,000 ms, which reaches the seeded Bookstore
 * Branch's own 30-second cadence boundary and produces an incidental real
 * sale — polluting assertions this file never intends to make about
 * Bookstore state. Pushing the seeded cadence record's next opportunity far
 * out keeps every scenario in this file free of that coincidental
...
describe('admitting a RACK-OS firmware installation', () => {
  ...
  it('fails closed on an incompatible Device and on one already running the release', () => {
    const phone = withArtifactOn(operating(PHONE_ID, '203.0.113.42', 'service-ssh-003'), PHONE_ID)
